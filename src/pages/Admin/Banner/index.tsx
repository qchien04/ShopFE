...
import HomepageConfigPage, { DEFAULT_SECTIONS } from "../Config/Homepageconfigpage";
...
const BannerManager = () => {
  ...
  const [productSections, setProductSections] = useState<ProductSectionConfig[]>([]);
  ...
  useEffect(() => {
    ...
    const quickTop = data.banners?.filter((b) => b.type === "quick-top") ?? [];
    const quickBot = data.banners?.filter((b) => b.type === "quick-bottom") ?? [];
    ...
    if (data.brandIds?.length) setBrandIds(data.brandIds);
    ...
    if (data.productSections && data.productSections.length > 0) {
      setProductSections(data.productSections);
    } else {
      setProductSections(DEFAULT_SECTIONS);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...(data || {}),
        banners: [...mainSlides, ...sideBanners, ...quickTopOption, ...quickBottomOption],
        categories,
        featuredPostIds: selectedPreIds,
        featuredPopularIds: selectedPopularIds,
        brandIds: brandIds,
      ...
      message.success("Đã lưu toàn bộ cấu hình trang chủ thành công!");
    } catch (err) {
  ...
  return (
    <div className="banner-manager">
      ...
      {/* ── Block Cấu hình Trang Chủ (Kéo thả, Section Sản phẩm) ── */}
      <div className="homepage-section-manager" style={{ marginTop: 40, borderTop: '2px dashed #eee', paddingTop: 20 }}>
        <HomepageConfigPage
